...
void free_plan(ExecutionPlan *plan) {
    ...
    for (int i = 0; i < plan->num_cmds; i++) {
        ...
        if (plan->cmds[i].append_file) free(plan->cmds[i].append_file);
...
void execute_plan(ExecutionPlan *plan) {
    ...
    int pipefds[pipes > 0 ? 2 * pipes : 1]; // array holding pipe file descriptors
    ...
    for (i = 0; i < plan->num_cmds; i++) {
        ...
        // fix: handle fork failure explicitly
        if (pid < 0) {
            perror("fork");
            exit(1);
        }
        ...
        if (pid == 0) { // inside child process
            ...
            // handle append redirection (>>) — append stdout to a file.
            // O_APPEND is used so existing content is preserved and new content is added at the end.
            if (plan->cmds[i].append_file) {
                int fd = open(plan->cmds[i].append_file, O_WRONLY | O_CREAT | O_APPEND, 0644);
    ...
    // parent closes all pipe ends and waits for children to finish
    // before returning to the main shell loop
